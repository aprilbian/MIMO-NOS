...
INTL = np.arange(V*I)    # interleaver between inner&outer code
DEINTL = np.argsort(INTL)            # deinterleaver
...
alpha = 0
PA_arr = np.array([2**(-alpha*v) for v in range(V)])
PA_arr = np.sqrt(PA_arr*V/np.sum(PA_arr))
...
def loop_kbest(y, H, k_best, lookup_table, ITER=V):
...
if __name__ == "__main__":
    ...
    SNR_db = [i+6 for i in range(3)]
    thres = 128

    name = 'MIMO_Cancel'+'_IS_RES_'+str(is_res)+'_V'+str(V) + '_M'+str(M)+'_Nt'+str(Nt)+'_Nr'+str(Nr)+'_F'+str(F)+\
        '_SNR'+str(train_snr)+'_alpha'+str(equ_alpha)+'_batchsz'+str(batch_size)

    name = 'MIMO_IS_RES_True_V8_M16_Nt2_Nr2_F16_SNR10_alpha0_batchsz1024'
    save_path = name+'/epoch10000.pth'

    model = MIMO_HDM_model(opt).to(opt['device'])
    #model = MIMO_cyc_model(opt).to(opt['device'])
    model.load_state_dict(torch.load(save_path, map_location=torch.device('cpu')))
    model.eval()

    lookup_table = construct_table(model)
    ...
    for snr in SNR_db:
        ...
        while perr<=thres:
            ...
            with torch.no_grad():
                ...
                idx_list, order, _ = loop_kbest(y,H,k_tree,lookup_table,V)
                _, dec_b = kbest_crc(idx_list, opt)

                if not np.array_equal(raw_bits,dec_b):
                    perr += 1
                    if(perr%40==0): print(perr)

        print('SNR = ', snr)
        print(perr/count)
        time_end = time.time()
        print(count)
        print('Time Per Iteration: ', (time_end-time_start)/count)